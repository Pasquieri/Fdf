...
/*   Updated: 2018/04/11 19:51:09 by cpalmier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	trace_droite(t_info info, t_coord coord1)
{
	...
	coord2 = recupts((info.pos_x + 1) * info.coef + info.right - (info.rota + 1) * info.h,
...
static void	trace_haut(t_info info, t_coord coord1)
{
	...
	coord2 = recupts((info.pos_x + 1) * info.coef + info.right - (info.rota * info.h),
			(info.pos_y - 1) * info.coef + info.down - (info.h* z));
	ft_trace_seg(info, coord1, coord2);
}
...
void	relie_pts_rotation_x(t_info info)
{
	...
	while (info.y--)
	{
		...
		while (++info.cmp_x < info.x)
		{
			...
			coord1 = recupts(info.pos_x * info.coef + info.right - (info.rota *info.h),
